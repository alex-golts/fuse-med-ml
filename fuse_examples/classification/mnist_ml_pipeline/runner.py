...
if __name__ == "__main__":
    ...
    NUM_REPETITIONS = 3 # different seeds

    TRAIN_COMMON_PARAMS['manager.num_cv_folds'] = NUM_CV_FOLDS
    TRAIN_COMMON_PARAMS['manager.num_repetitions'] = NUM_REPETITIONS

    RUNNING_MODES = ['train', 'infer', 'eval', 'infer_ensembled', 'eval_ensembled']  # Options: 'train', 'infer', 'eval'
    
    #train_val_splits = create_data_splits(paths=PATHS, num_folds=NUM_CV_FOLDS)
    # train
    ...
    kfold = KFold(n_splits=NUM_CV_FOLDS, shuffle=True, random_state=123)
    for fold, (train_ids, val_ids) in enumerate(kfold.split(torch_train_dataset)):

        if 'train' in RUNNING_MODES:
            run_train(paths=PATHS, train_params=TRAIN_COMMON_PARAMS, ids=train_ids)

        # infer
        if 'infer' in RUNNING_MODES:
            run_infer(paths=PATHS, infer_common_params=INFER_COMMON_PARAMS, split=split)

        # eval
        if 'eval' in RUNNING_MODES:
            run_eval(paths=PATHS, eval_common_params=EVAL_COMMON_PARAMS, split=split)
